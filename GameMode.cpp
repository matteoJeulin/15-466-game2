...
GameMode::GameMode() : scene(*fish_scene) {
	...
	for (auto &transform : scene.transforms) {
		...
		else if (transform.name == "BackFin") back_fin = &transform;
		else if (transform.name == "Box") box = &transform;
	}
	if (body == nullptr) throw std::runtime_error("Hip not found.");
	if (back_fin == nullptr) throw std::runtime_error("Upper leg not found.");
...
void GameMode::update(float elapsed) {
	...
	back_fin->rotation = back_fin_base_rotation * glm::angleAxis(
		(float(M_PI) / 4) * std::sin(wobble * 2.0f * float(M_PI)),
		glm::vec3(0.0f, 1.0f, 0.0f)
	);

	// move camera:
	{
		constexpr float PlayerAcceleration = 5.0f;

		//combine inputs into a move:
		if (left.pressed && !right.pressed) playerSpeed.x = std::max(playerSpeed.x - PlayerAcceleration * elapsed, -30.0f);
		if (!left.pressed && right.pressed) playerSpeed.x = std::min(playerSpeed.x + PlayerAcceleration * elapsed, 30.0f);
		if (down.pressed && !up.pressed) playerSpeed.y = std::max(playerSpeed.y - PlayerAcceleration * elapsed, -30.0f);
		if (!down.pressed && up.pressed) playerSpeed.y = std::min(playerSpeed.y + PlayerAcceleration * elapsed, 30.0f);
		...
		glm::mat4x3 frame = body->make_parent_from_local();
		...
		body->position += playerSpeed.x * frame_right * elapsed + playerSpeed.y * frame_forward * elapsed + playerSpeed.z * frame_up * elapsed;
	}
